...
/*bool Bank::checkSame(Bank &other){
    if (accountsVector.size() != other.accountsVector.size()) {
        cout << "Size mismatch: " << accountsVector.size() << " vs " << other.accountsVector.size() << endl;
        return false;
    }

    for (size_t i = 0; i < accountsVector.size(); ++i) {
        if (!accountsVector[i]->equalTo(other.accountsVector[i])) {
            cout << "Account mismatch at index " << i << ":" << endl;
            cout << "Original: " << accountsVector[i]->toString() << endl;
            cout << "Backup: " << other.accountsVector[i]->toString() << endl;
            return false;
        }
    }
